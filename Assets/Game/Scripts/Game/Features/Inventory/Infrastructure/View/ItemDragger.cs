...
namespace Game.Features.Inventory.Infrastructure.View
{
    public class ItemDragger
    {
        ...
        public ItemDragger(IPlayerInput input, Camera gameCamera, IEnumerable<ItemContainer> containers, RectTransform draggableSpaceRect)
        {
            _input = input;
        ...
        {
            if(RectTransformUtility.RectangleContainsScreenPoint(_draggableSpaceRect, _input.CursorPos, _gameCamera) && 
               RectTransformUtility.ScreenPointToWorldPointInRectangle(_draggableSpaceRect, _input.CursorPos, _gameCamera, out var worldPoint))
        ...
        private void DetectHoveredContainer()
        {
            ...
            {
                if (!container.TryProjectItem(_draggedInventoryItem, _input.CursorPos, _gameCamera, out var insertItemCommand)) 
                    continue;
        ...
        private void DetectHoveredItem()
        {
            foreach (var container in _containers)
            {
                if (!container.TryRaycastItem(_input.CursorPos, _gameCamera, out var item, out var extractItemCommand)) 
                    continue;
